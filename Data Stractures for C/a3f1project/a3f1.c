...
int main(){
typos_synolou katholiko1,S,T,E,Z;
Dimiourgia(katholiko1);
Dimiourgia(S);
Dimiourgia(T);
Dimiourgia(E);
Dimiourgia(Z);
for(char i='A';i<='Z';i++){
    Eisagogi(i,katholiko1);
}                                     //a
for(char i='A';i<='D';i++){
  Eisagogi(i,S);
}
...
void check(typos_synolou S,typos_synolou T){
if(IsaSynola(S,T)){
    printf("'ISA SYNOLA'\n"); }
else if(Yposynolo(S,T)){
    printf("'S YPOSYNOLO T \n'");
}
else if(Yposynolo(T,S)){
